...
class postalController extends Controller {
	...
	public function listAll($page = 1,$search = ""){
		...
		$toReturn["postal"] = $toReturn["postal"]->select('id','refno','postal_type','postal_from','postal_to','postal_time','attachment')->take('20')->skip(20* ($page - 1) )->get()->toArray();
	...
	public function create(){
		...
		if (\Input::hasFile('attachment')) {
			...
			$fileInstance->move('uploads/postal/',$newFileName);
	...
	public function edit($id){
		...
		if (\Input::hasFile('attachment')) {
			...
			if($postal->attachment != ""){
				@unlink("uploads/postal/".$postal->attachment);
			}
	...
	public function delete($id){
		...
		if ( $postDelete = \postal::where('id', $id)->first() ){
        		if($postDelete->attachment != ""){ @unlink('uploads/postal/'.$postDelete->attachment); }

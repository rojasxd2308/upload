...
class visitorsController extends Controller {
	...
	public function listAll($page = 1,$search = ""){
		...
		$toReturn["visitors"] = $toReturn["visitors"]->select('id','pass_id','fullName','phoneNo','email','id_pass_id','usr_type','comp_name','student','to_meet','purpose','check_in','check_out')->take('20')->skip(20* ($page - 1) )->get()->toArray();
	...
	public function create(){
		...
		if (\Input::hasFile('docs')) {
			...
			$fileInstance->move('uploads/visitors/',$newFileName);
	...
	public function edit($id){
		...
		if (\Input::hasFile('docs')) {
			...
			if($visitors->docs != ""){
				@unlink("uploads/visitors/".$visitors->docs);
			}
	...
	public function delete($id){
		...
		if ( $postDelete = \visitors::where('id', $id)->first() ){
        		if($postDelete->docs != ""){ @unlink('uploads/visitors/'.$postDelete->docs); }
